...
| `.../FBX/RU_SSN_Akula_PERISCOPE.fbx` | 潜望镜（`04_MASTS` 里的 `Mast_03`，最细最高的桅杆，无独立头部对象） | 包围盒中心 (2.40, 0.35, 15.59)，沿 Z 升降 |

枢轴规则：舵面取各组包围盒的 `max_x`（根弦前缘），螺旋桨取桨毂包围盒中心，潜望镜取自身包围盒中心（升降件不需要铰链）。所有部件导出时都把几何平移到枢轴、对象变换归零，所以 UE 侧组件只要落在铰链上就能正确旋转。

潜望镜高 9.4 m，收放行程取自身高度——完全降下时桅顶落在围壳顶面之内，从外面看不见。只有 `Periscope` 深度层会升起，其余层收下，升降各约 2.5 秒。
