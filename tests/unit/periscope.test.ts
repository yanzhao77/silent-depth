...
import type { Contact, EnemyShip, MissionDef, PeriscopePublicState, PlayerInputs, SubmarineState } from '../../src/core/types'
import {
...
describe('periscope state machine', () => {
  it('1. too deep: canRaise=false reason tooDeep; autoSurface rises (SURFACING), never immediately RAISED', () => {
    const ctx = makeCtx({ player: makePlayer({ depthLayer: 'Medium', targetDepthLayer: 'Medium' }) })
  ...
  it('1b. autoSurface=false rejects the raise from a deeper layer with cannotRaise tooDeep', () => {
    ...
    const ctx = makeCtx({ player: makePlayer({ depthLayer: 'Deep', targetDepthLayer: 'Deep' }), balance: custom })
    tick(ctx, { periscopeEdge: true })
  ...
  it('1c. Surface layer: canRaise=false reason wrongLayer; raise rejected (wrongLayer)', () => {
    const ctx = makeCtx({ player: makePlayer({ depthLayer: 'Surface', targetDepthLayer: 'Surface' }) })
    tick(ctx) // first tick populates canRaise/cannotRaiseReason
    expect(ctx.periscope!.cannotRaiseReason).toBe('wrongLayer')
    tick(ctx, { periscopeEdge: true })
    expect(ctx.periscope!.state).toBe('SUBMERGED')
    expect(ctx.bus.getLog().filter((e) => e.type === 'periscope.cannotRaise')[0]!.payload).toEqual({ reason: 'wrongLayer' })
  })
...
describe('visual observation (the core reward)', () => {
  it('4+5. UNKNOWN contact in view → OBSERVING, CONFIRMED with ground-truth type', () => {
    ...
    expect(ctx.bus.getLog().filter((e) => e.type === 'periscope.visualContact')[0]!.payload).toEqual({
      contactId: 'C-01',
  ...
  it('FOV and range gating: target outside the cone or beyond visual range is not observed', () => {
    ...
    const offFov = makeCtx({ contacts: [makeContact()], enemies: [makeEnemy({ position: { x: 1, y: 1 } })] })
    ...
    const far = makeCtx({ contacts: [makeContact()], enemies: [makeEnemy({ position: { x: 0, y: 6 } })] })
    raiseToRaised(far)
    tick(far)
  ...
  it('6. fire solution: ESTIMATED → VISUAL CONFIRMED (confidence penalty removed, HP up)', () => {
    ...
    const imprecise = makeContact({ state: 'TRACKED', rangeKm: 2, headingEstimateDeg: 270, speedEstimateKt: 9, confidence: 70, classification: 'Merchant' })
    ...
    expect(visual.hitProbability).toBeCloseTo(withConfPen + BALANCE.hitProbability.confidencePen['70']!, 9)
    // and after the periscope upgrade (98 conf, precise data) the visual solve is near-ideal
    const observed = makeContact({ state: 'CONFIRMED', rangeKm: 1, headingEstimateDeg: 270, speedEstimateKt: 9, confidence: 98, classification: 'Merchant', visuallyConfirmed: true })
...
describe('lock & fire', () => {
  ...
  it('8. fire while locked + raised: torpedo fired + raised detection bonus', () => {
    ...
    const expected = BALANCE.detection.sources.torpedoFired + BALANCE.periscope.torpedoFiredWhileRaisedBonus
  ...
  it('firing while submerged does NOT get the raised bonus', () => {
    ...
    ctx.bus.setSimTime(ctx.simTime)
    combatSystem(ctx)
    expect(ctx.player.detection).toBeCloseTo(detectionBefore + BALANCE.detection.sources.torpedoFired, 9)
  })
})
...
describe('exposure & risk', () => {
  it('10. exposure bands LOW→MEDIUM→HIGH→CRITICAL with per-tick detection raises', () => {
    ...
    const bands = ctx.bus.getLog().filter((e) => e.type === 'periscope.exposure').map((e) => e.payload!.band)
...
describe('emergency dive', () => {
  it('13. diveEdge while raised lowers at the boosted rate; submarine pays battery + event', () => {
    ...
    expect(ctx.player.battery).toBeCloseTo(100 - BALANCE.battery.emergencyDiveCostPercent - bandDrain, 9)
...
describe('determinism & pause', () => {
  it('same seed + same inputs → identical periscope state, contacts, and events', () => {
    const run = (): string => {
      ...
      return JSON.stringify({ periscope: ctx.periscope, contact: ctx.contacts[0], detection: ctx.player.detection, events: ctx.bus.getLog() })
    }
    expect(run()).toBe(run())
  })
  ...
  it('periscope state freezes while the mission is paused (engine level)', () => {
    ...
    for (let t = 0; t < 119; t++) snap = step(handle, FIXED_DT, { ...IDLE_INPUT, depthLayerTarget: 'Periscope' })
  ...
  it('engine-level auto-surface: Medium → SURFACING → submarine rises → RAISED', () => {
    ...
    for (let t = 0; t < 119; t++) snap = step(handle, FIXED_DT, { ...IDLE_INPUT, depthLayerTarget: 'Medium' })
    ...
    expect(snap.playerSub.depthLayer).toBe('Periscope')
    for (let t = 0; t < 100 && snap.periscope.state !== 'RAISED'; t++) snap = step(handle, FIXED_DT, IDLE_INPUT)
    expect(snap.periscope.state).toBe('RAISED')
  })
})
