...
describe('M02 scripted torpedo victory is repeatable (same seed → same outcome)', () => {
  ...
  function play(): GameSnapshot {
    ...
    for (let t = 0; t < 120000; t++) {
      ...
      const wantRangePing = snap.playerSub.pingCooldown <= 0 && t - lastPingAt >= 3000 && !last.ping
...
describe('F9 escape event fires exactly once per game', () => {
  ...
  function play(): GameSnapshot {
    ...
    for (let t = 0; t < 2000; t++) {
      ...
      const awayBearing =
        escorts.length > 0 ? compassBearing(snap.playerSub.position, escorts[0]!.position) + 180 : snap.playerSub.headingDeg
      const inputs: PlayerInputs = {
        throttle: 4,
