...
function setup(
  depthTarget: 'Surface' | 'Periscope',
  battery: number,
): { step: (i: PlayerInputs) => ReturnType<typeof step> } {
  ...
  let snap = step(h, FIXED_DT, {
    throttle: 0,
    rudder: 0,
    depthLayerTarget: depthTarget,
    silentRunning: false,
    ping: false,
    fireTorpedo: null,
    decoy: false,
  ...
  for (
    ;
    t < 400 && (snap.state !== 'MISSION_RUNNING' || snap.playerSub.depthLayer !== depthTarget);
    t++
  ) {
    snap = step(h, FIXED_DT, {
      throttle: 0,
      rudder: 0,
      depthLayerTarget: depthTarget,
      silentRunning: false,
