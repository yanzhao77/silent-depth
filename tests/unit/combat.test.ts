...
import type { Contact, EnemyShip, MissionDef, PlayerInputs, SubmarineState, Torpedo } from '../../src/core/types'
...
function makeMission(): MissionDef {
  return {
    ...
    fleet: { headingDeg: 90, speedKt: 9, formation: '2x2', colSpacingM: 500, rowSpacingM: 400, patrolBehavior: 'figure8' },
...
function makeCtx(opts: CtxOptions = {}): SystemContext {
  ...
  return {
    ...
    decoys: [],
    missionStatus: { missionId: mission.id, phase: 'running', objectives: [], escaped: false, forcedSurface: false },
...
describe('fire control (F6/F7)', () => {
  it('HP is the balance base at ideal geometry (range 1, AOB 90, slow, confident, Merchant)', () => {
    ...
    const s = solution({ bearingDeg: 90, headingEstimateDeg: 0, speedEstimateKt: 4, confidence: 95 })
  ...
  it('applies the range penalty from balance (piecewise)', () => {
    ...
    expect(solution({ rangeKm: 6 }).hitProbability).toBeCloseTo(0.85 - BALANCE.hitProbability.rangePen.le6km, 9) // 0.50
    // unknown range → worst case
    expect(solution({ rangeKm: null }).hitProbability).toBeCloseTo(0.85 - BALANCE.hitProbability.rangePen.le6km, 9)
  })

  it('applies the AOB penalty table (0 head-on … 90 broadside)', () => {
    // AOB 0: target heading straight at the observer
    expect(solution({ bearingDeg: 90, headingEstimateDeg: 270 }).hitProbability).toBeCloseTo(0.85 - BALANCE.hitProbability.aobPen['0deg'], 9)
    // AOB 45
    expect(solution({ bearingDeg: 90, headingEstimateDeg: 225 }).hitProbability).toBeCloseTo(0.85 - BALANCE.hitProbability.aobPen['45deg'], 9)
    // AOB 90 (ideal)
    expect(solution({ bearingDeg: 90, headingEstimateDeg: 0 }).hitProbability).toBeCloseTo(0.85, 9)
    // unknown course → mid default ('45deg')
    expect(solution({ headingEstimateDeg: null }).hitProbability).toBeCloseTo(0.85 - BALANCE.hitProbability.aobPen['45deg'], 9)
  })

  it('applies the target-speed and confidence penalties', () => {
    expect(solution({ speedEstimateKt: 25 }).hitProbability).toBeCloseTo(0.85 - BALANCE.hitProbability.targetSpeedPen['20ktPlus'], 9)
    expect(solution({ speedEstimateKt: 10 }).hitProbability).toBeCloseTo(0.85 - BALANCE.hitProbability.targetSpeedPen['10kt'], 9)
  ...
  it('applies the maneuver penalty by classification and clamps to [0.05, 0.95]', () => {
    expect(solution({ classification: 'Destroyer' }).hitProbability).toBeCloseTo(0.85 - BALANCE.hitProbability.maneuverPen['Destroyer']!, 9)
    expect(solution({ classification: 'Unknown' }).hitProbability).toBeCloseTo(0.85, 9)
    const worst = solution({ rangeKm: 6, headingEstimateDeg: 270, speedEstimateKt: 25, confidence: 5, classification: 'Destroyer' })
  ...
  it('computes the F6 lead angle and firing bearing (broadside target)', () => {
    ...
    const expectedLead = (Math.atan2(9 * Math.sin(Math.PI / 2), 9 * Math.cos(Math.PI / 2) + BALANCE.torpedo.speedKt) * 180) / Math.PI
...
describe('torpedo lifecycle', () => {
  ...
  it('fires, runs straight, and HITs a stationary target: damage applied', () => {
    ...
    const ctx = makeCtx({ dt: 0.5, enemies: [ship], contacts: [makeContact({ rangeKm: 0.5 })], player: singleTubePlayer() })
    ...
    expect(ship.hull).toBeGreaterThanOrEqual(200 - BALANCE.torpedo.damageBase - BALANCE.torpedo.damageSpread)
    expect(ship.hull).toBeLessThanOrEqual(200 - BALANCE.torpedo.damageBase + BALANCE.torpedo.damageSpread)
  ...
  it('MISSED on a near miss (closest pass 40–120 m)', () => {
    ...
    const ctx = makeCtx({ dt: 0.5, enemies: [ship], contacts: [makeContact({ rangeKm: 2 })], player: singleTubePlayer() })
    tick(ctx, { inputs: { fireTorpedo: 'C-01' } })
...
describe('depthChargeDamage', () => {
  it('resolves the direct/near/far tiers and the Deep-layer ×1.5 factor', () => {
    expect(depthChargeDamage(30, 'Shallow', BALANCE)).toBe(BALANCE.weapons.depthCharge.directDamage) // 35
    expect(depthChargeDamage(80, 'Shallow', BALANCE)).toBe(BALANCE.weapons.depthCharge.nearMissDamage) // 20
    ...
    expect(depthChargeDamage(30, 'Deep', BALANCE)).toBe(Math.round(35 * BALANCE.depthLayers.Deep.dcDamageFactor)) // 53
...
describe('pending damage drain', () => {
  it('applies AI depth-charge damage once per tick and never leaks', () => {
    ...
    const player = makePlayer({ position: { x: 10.001, y: 10 }, detection: 0, depthLayer: 'Deep', hull: 100 })
    ...
    const expected = 100 - Math.round(BALANCE.weapons.depthCharge.directDamage * BALANCE.depthLayers.Deep.dcDamageFactor)
    expect(ctx.player.hull).toBe(expected) // 100 − 53 = 47
    expect(ctx.bus.getLog().filter((e) => e.type === 'sub.damaged' && e.payload!.source === 'depthCharge')).toHaveLength(1)
...
describe('detection sinks (pure)', () => {
  it('STOPPED/SILENT + silent running decay rates', () => {
    expect(stoppedSilentPerSec(makePlayer({ speedBand: 'STOPPED', silentRunning: true }), BALANCE)).toBe(BALANCE.detection.sinks.stoppedSilentPerSec)
    expect(stoppedSilentPerSec(makePlayer({ speedBand: 'STOPPED', silentRunning: false }), BALANCE)).toBe(0)
    expect(silentSilentPerSec(makePlayer({ speedBand: 'SILENT', silentRunning: true }), BALANCE)).toBe(BALANCE.detection.sinks.silentSilentPerSec)
    expect(silentSilentPerSec(makePlayer({ speedBand: 'CRUISE', silentRunning: true }), BALANCE)).toBe(0)
  })

  it('dive sink fires only when entering Medium/Deep from above', () => {
    expect(diveSink('Shallow', 'Medium', BALANCE)).toBe(BALANCE.detection.sinks.diveSurfaceToMedium)
...
describe('detection system', () => {
  ...
  it('applies the decoy-launch sink on launch and the dive sink on entering Medium', () => {
    ...
    tickDetection(ctx, 1)
    expect(ctx.player.detection).toBeCloseTo(60 - BALANCE.detection.sinks.decoyLaunch - BALANCE.detection.sinks.diveSurfaceToMedium, 9)
  })
...
describe('combat determinism', () => {
  it('same seed + same inputs → identical state, torpedoes, and events', () => {
    const run = (): string => {
      ...
      const ctx = makeCtx({ dt: 0.5, enemies: [ship], contacts: [makeContact({ rangeKm: 0.5, trueShipId: 'E-01' })] })
      ...
      tick(ctx, { inputs: { fireTorpedo: 'C-01' } })
      return JSON.stringify({ player: ctx.player, ship, torpedoes: ctx.torpedoes, stats: ctx.stats, events: ctx.bus.getLog() })
    }
    expect(run()).toBe(run())
  })
})
