    ...
    for (const name of SFX_NAMES) {
      ...
      if (params.filter) {
        ...
        if (params.filter.q !== undefined)
          expect(params.filter.q, `${name} filter q`).toBeGreaterThan(0);
      }
      if (params.filter2) {
      ...
      if (params.noise2)
        expect(['white', 'pink', 'brown'], `${name} noise2`).toContain(params.noise2);
      if (params.waveform) {
        expect(['sine', 'square', 'sawtooth', 'triangle'], `${name} waveform`).toContain(
          params.waveform,
        );
      }
      if (params.bandGains) {
...
describe('event → SFX wiring', () => {
  ...
  it('every mapped engine event exists in the architecture event catalogue', async () => {
    ...
    for (const ev of Object.keys(EVENT_SFX_MAP)) {
      ...
      expect(catalogue, `mapped engine event '${ev}' missing from EventType catalogue`).toContain(
        ev,
      );
    }
  });
  ...
  it('detection.threshold is gated ≥60 in the engine (GAME_ARCHITECTURE §14)', () => {
    ...
    expect(() =>
      engine.onEngineEvent({ type: 'detection.threshold', payload: { detection: 40, band: 40 } }),
    ).not.toThrow();
    expect(() =>
      engine.onEngineEvent({ type: 'detection.threshold', payload: { detection: 70, band: 60 } }),
    ).not.toThrow();
    engine.dispose();
  });
  ...
  it('periscope.exposure is band-gated (HIGH/CRITICAL) + throttled ≥2.5s in the engine', () => {
    ...
    expect(() =>
      engine.onEngineEvent({ type: 'periscope.exposure', payload: { band: 'LOW' } }),
    ).not.toThrow();
    expect(() =>
      engine.onEngineEvent({ type: 'periscope.exposure', payload: { band: 'MEDIUM' } }),
    ).not.toThrow();
    expect(() =>
      engine.onEngineEvent({ type: 'periscope.exposure', payload: { band: 'HIGH' } }),
    ).not.toThrow();
    expect(() =>
      engine.onEngineEvent({ type: 'periscope.exposure', payload: { band: 'CRITICAL' } }),
    ).not.toThrow();
...
describe('audio engine in Node (no AudioContext)', () => {
  ...
  it('setVolume / setWeather / onEngineEvent are safe no-ops in Node', () => {
    ...
    expect(() =>
      engine.onEngineEvent({
        type: 'torpedo.fired',
        payload: { tubeId: 'T1', targetContactId: 'C-01' },
      }),
    ).not.toThrow();
    expect(() =>
      engine.onEngineEvent({
        type: 'ship.sunk',
        payload: { shipId: 'E-1', shipClass: 'Merchant' },
      }),
    ).not.toThrow();
    expect(() =>
      engine.onEngineEvent({
        type: 'sub.speedChanged',
        payload: { band: 'FULL', speedKt: 18, noise: 60 },
      }),
    ).not.toThrow();
    ...
    expect(() =>
      engine.onEngineEvent({ type: 'ui.click', payload: { elementId: 'btn' } }),
    ).not.toThrow();
    ...
    expect(() =>
      engine.onEngineEvent({
        type: 'periscope.visualContact',
        payload: { contactId: 'C-01', classification: 'Merchant' },
      }),
    ).not.toThrow();
    expect(() =>
      engine.onEngineEvent({ type: 'periscope.locked', payload: { contactId: 'C-01' } }),
    ).not.toThrow();
    expect(() =>
      engine.onEngineEvent({ type: 'periscope.exposure', payload: { band: 'CRITICAL' } }),
    ).not.toThrow();
