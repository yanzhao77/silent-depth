...
import type { MissionDef, PlayerInputs, SubmarineState } from '../../src/core/types';
...
describe('F1 noise', () => {
  ...
  it('applies depth-layer noise mods (B2) and the F2 transition mean', () => {
    ...
    const transit = computeNoise({
      band: 'SILENT',
      speedKt: 3,
      depthLayer: 'Shallow',
      transitionLayer: 'Deep',
      hull: 100,
      weather: 'Clear',
      balance: b,
    });
  ...
  it('adds hull-damaged and storm-surface bonuses', () => {
    ...
    const stormSurface = computeNoise({
      band: 'SILENT',
      speedKt: 3,
      depthLayer: 'Surface',
      transitionLayer: null,
    ...
    const clearSurface = computeNoise({
      band: 'SILENT',
      speedKt: 3,
      depthLayer: 'Surface',
      transitionLayer: null,
...
describe('battery', () => {
  ...
  it('recharges at Surface and gains the Deep ballast bonus', () => {
    ...
    expect(surface.player.battery).toBeCloseTo(
      50 + (b.battery.surfaceFastChargePerSec - b.speedBands.STOPPED.batteryDrainPerSec),
      6,
    );
    ...
    expect(deep.player.battery).toBeCloseTo(
      50 + (b.depthLayers.Deep.extraBatteryPerSec - b.speedBands.STOPPED.batteryDrainPerSec),
      6,
    );
  });
...
describe('low battery', () => {
  ...
  it('caps speed intent at SILENT max and halves the rudder rate', () => {
    ...
    expect(ctx.player.headingDeg).toBeCloseTo(
      loadBalance().rudder.turnRateDegPerSec *
        loadBalance().rudder.lowBatteryTurnRateFactor *
        FIXED_DT,
      9,
    );
    ...
    expect(normal.player.headingDeg).toBeCloseTo(
      loadBalance().rudder.turnRateDegPerSec * FIXED_DT,
      9,
    );
  });
...
describe('forced surface', () => {
  it('triggers once at 0 battery: Surface layer, detection 100, event', () => {
    ...
    for (let i = 0; i < 100; i++) {
      tick(ctx, { inputs: { throttle: 0 } });
      if (ctx.missionStatus.forcedSurface) {
        forced = true;
        break;
      }
    }
...
describe('depth transitions', () => {
  it('takes 3 s per layer (F2) and emits sub.depthChanged on completion', () => {
    ...
    expect(ctx.player.depthTransitionT).toBeCloseTo(
      2 * b.depthTransitionSecondsPerLayer - FIXED_DT,
      9,
    );
...
describe('out of bounds & hull', () => {
  ...
  it('applyHullDamage reduces hull, clamps at 0, and emits sub.damaged', () => {
    ...
    expect(ctx.bus.getLog()).toEqual([
      expect.objectContaining({
        type: 'sub.damaged',
        payload: { source: 'depthCharge', amount: 25, hullLeft: 75 },
      }),
    ]);
