...
import type { AiStateTimers, AiThresholds, AiTriggers } from '../../src/ai/aiState'
import {
  KT_TO_KM_S,
  applyDamage,
  angleDiffDeg,
...
import type { EscortTickCtx } from '../../src/ai/escort'
import {
...
import {
  aiSystem,
  drainAiPendingDamage,
  resetAiPendingOutput,
} from '../../src/ai/ai'
import { createShipRuntime } from '../../src/ai/ship'
import type { AiShipRuntime } from '../../src/ai/ship'
...
function makeDecoy(overrides: Partial<Decoy> = {}): Decoy {
  return {
    id: 'D-01',
    position: { x: 5, y: 5 },
    ageS: 1,
...
describe('aiState machine (GAME_DESIGN §6.1)', () => {
  ...
  it('exposes the six canonical states in order', () => {
    expect(AI_STATES).toEqual(['NORMAL', 'SUSPICIOUS', 'ALERT', 'SEARCHING', 'HUNTING', 'LOST_CONTACT'])
  })
  ...
  it('NORMAL → SUSPICIOUS on player noise or ping, else stays', () => {
    ...
    expect(evaluateAiState('NORMAL', triggers({ noiseSensed: true }), ZERO_TIMERS, th).next).toBe('SUSPICIOUS')
    expect(evaluateAiState('NORMAL', triggers({ pingHeard: true }), ZERO_TIMERS, th).next).toBe('SUSPICIOUS')
  })

  it('NORMAL/SUSPICIOUS → ALERT on torpedo heard within 10 km (§7.2 early warning)', () => {
    expect(evaluateAiState('NORMAL', triggers({ torpedoNearKm: 8 }), ZERO_TIMERS, th).next).toBe('ALERT')
    expect(evaluateAiState('SUSPICIOUS', triggers({ torpedoNearKm: 5 }), ZERO_TIMERS, th).next).toBe('ALERT')
    // beyond hearing range → no escalation
    expect(evaluateAiState('NORMAL', triggers({ torpedoNearKm: 11 }), ZERO_TIMERS, th).next).toBe('NORMAL')
  })

  it('SUSPICIOUS → ALERT when detection ≥ 40', () => {
    expect(evaluateAiState('SUSPICIOUS', triggers({ detection: 39.9 }), ZERO_TIMERS, th).next).toBe('SUSPICIOUS')
    expect(evaluateAiState('SUSPICIOUS', triggers({ detection: 40 }), ZERO_TIMERS, th).next).toBe('ALERT')
  })
  ...
  it('SUSPICIOUS → LOST_CONTACT after 60 s without contact', () => {
    const timers: AiStateTimers = { ...ZERO_TIMERS, suspiciousNoContactS: SUSPICIOUS_TIMEOUT_S - 0.01 }
  ...
  it('ALERT → SEARCHING on torpedo within 3 km or explosion heard', () => {
    ...
    expect(evaluateAiState('ALERT', triggers({ torpedoNearKm: 3 }), ZERO_TIMERS, th).next).toBe('SEARCHING')
    expect(evaluateAiState('ALERT', triggers({ torpedoNearKm: 2.9 }), ZERO_TIMERS, th).next).toBe('SEARCHING')
    expect(evaluateAiState('ALERT', triggers({ explosionHeard: true }), ZERO_TIMERS, th).next).toBe('SEARCHING')
  })

  it('SEARCHING → HUNTING on detection ≥ 60 or ping confirm < 1.5 km', () => {
    expect(evaluateAiState('SEARCHING', triggers({ detection: 59.9 }), ZERO_TIMERS, th).next).toBe('SEARCHING')
    expect(evaluateAiState('SEARCHING', triggers({ detection: 60 }), ZERO_TIMERS, th).next).toBe('HUNTING')
    expect(
      evaluateAiState('SEARCHING', triggers({ detection: 30, lastPingHitRangeKm: 1.49 }), ZERO_TIMERS, th).next,
    ).toBe('HUNTING')
  })
  ...
  it('SEARCHING can never escalate to HUNTING once hunting is disabled (ammo exhausted)', () => {
    expect(
      evaluateAiState('SEARCHING', triggers({ detection: 100, huntingDisabled: true }), ZERO_TIMERS, th).next,
    ).toBe('SEARCHING')
  })

  it('HUNTING → SEARCHING after detection < 40 for 30 s', () => {
    const timers: AiStateTimers = { ...ZERO_TIMERS, huntingBelow40S: 30 }
    expect(evaluateAiState('HUNTING', triggers({ detection: 39 }), timers, th).next).toBe('SEARCHING')
    expect(evaluateAiState('HUNTING', triggers({ detection: 39 }), ZERO_TIMERS, th).next).toBe('HUNTING')
    expect(evaluateAiState('HUNTING', triggers({ detection: 40 }), timers, th).next).toBe('HUNTING')
  })

  it('HUNTING → SEARCHING immediately when ammo is exhausted', () => {
    expect(evaluateAiState('HUNTING', triggers({ huntingDisabled: true, detection: 90 }), ZERO_TIMERS, th).next).toBe(
      'SEARCHING',
    )
  })
  ...
  it('LOST_CONTACT → NORMAL after 60 s at post', () => {
    ...
    expect(evaluateAiState('LOST_CONTACT', triggers({ atPost: true }), timers, th).next).toBe('NORMAL')
    expect(evaluateAiState('LOST_CONTACT', triggers({ atPost: false }), timers, th).next).toBe('LOST_CONTACT')
    expect(evaluateAiState('LOST_CONTACT', triggers({ atPost: true }), ZERO_TIMERS, th).next).toBe('LOST_CONTACT')
  })

  it('LOST_CONTACT re-contact en route → SUSPICIOUS, or ALERT when detection ≥ 40', () => {
    expect(evaluateAiState('LOST_CONTACT', triggers({ noiseSensed: true }), ZERO_TIMERS, th).next).toBe('SUSPICIOUS')
    expect(evaluateAiState('LOST_CONTACT', triggers({ pingHeard: true }), ZERO_TIMERS, th).next).toBe('SUSPICIOUS')
    expect(evaluateAiState('LOST_CONTACT', triggers({ detection: 41 }), ZERO_TIMERS, th).next).toBe('ALERT')
  })
})
...
describe('ship primitives (F3, kinematics, damage)', () => {
  ...
  it('F3 is monotonic in player noise', () => {
    ...
    const rates = [20, 50, 80].map((noise) =>
      passiveDetectionRate(escort, makePlayer({ position: { x: 1, y: 0 }, noise, depthLayer: 'Surface' }), BALANCE, 'Clear'),
    )
  ...
  it('F3 is monotonic in the depth factor (Surface > Shallow > Deep)', () => {
    ...
    const surface = passiveDetectionRate(escort, makePlayer({ ...base, depthLayer: 'Surface' }), BALANCE, 'Clear')
    const shallow = passiveDetectionRate(escort, makePlayer({ ...base, depthLayer: 'Shallow' }), BALANCE, 'Clear')
    const deep = passiveDetectionRate(escort, makePlayer({ ...base, depthLayer: 'Deep' }), BALANCE, 'Clear')
...
describe('convoy formation (GAME_DESIGN §6.3)', () => {
  ...
  it('merchant evade: torpedo-targeted triggers 45° turn for 30 s (30% roll)', () => {
    ...
    expect(rt.evadeHeadingDeg).toBe(normalizeDeg(headingBefore - BALANCE.enemyAI.merchant.evadeTurnDeg))
  ...
  it('merchant in formation steers toward its slot at fleet speed', () => {
    ...
    expect(ship.headingDeg).toBeCloseTo(normalizeDeg(90 + Math.sign(diff) * Math.min(Math.abs(diff), 4)), 6)
...
describe('aiSystem escort perception → state (GAME_DESIGN §6.1)', () => {
  ...
  it('SUSPICIOUS → ALERT on 2 consecutive own-ping hits with range', () => {
    ...
    expect(escort.aiState).toBe('SUSPICIOUS')
    expect(ctx.player.detection).toBeGreaterThanOrEqual(BALANCE.enemyAI.activePing.detectionGainOnPlayer)
    runTicks(ctx, 100) // second ping at t≈4.1 → 2 consecutive hits → ALERT
    expect(escort.aiState).toBe('ALERT')
  })

  it('ALERT → SEARCHING when a torpedo runs within 3 km', () => {
    const escort = makeEnemy({ position: { x: 10, y: 10 }, aiState: 'ALERT', lkp: { x: 10, y: 10, errorKm: 0 } })
  ...
  it('SEARCHING → HUNTING when detection ≥ 60', () => {
    const escort = makeEnemy({ position: { x: 10, y: 10 }, aiState: 'SEARCHING', lkp: { x: 10, y: 10, errorKm: 0 } })
...
describe('aiSystem attacks and events', () => {
  it('destroyer depth-charge volley cadence: 6 per round at 3 s, 20 s round gap, ammo spent', () => {
    ...
    const player = makePlayer({ position: { x: 10.001, y: 10 }, noise: 0, detection: 80, depthLayer: 'Deep' })
  ...
  it('depth-charge exhaustion disables HUNTING forever (§6.1 escape window)', () => {
    ...
    const player = makePlayer({ position: { x: 10.001, y: 10 }, noise: 0, detection: 90, depthLayer: 'Deep' })
    const ctx = makeCtx({ enemies: [escort], player })
    runTicks(ctx, 2)
  ...
  it('escort active-ping cooldown is published on the public view', () => {
    ...
    expect(escort.activePingCooldown).toBeLessThanOrEqual(BALANCE.enemyAI.activePing.suspiciousIntervalSeconds)
  })

  it('F3 detection deltas accumulate on the player meter and clamp at 100', () => {
    const escort = makeEnemy({ position: { x: 10, y: 10 } })
    const player = makePlayer({ position: { x: 10, y: 10 }, noise: 100, depthLayer: 'Surface', detection: 99.9 })
    const ctx = makeCtx({ enemies: [escort], player })
    runTicks(ctx, 5)
  ...
  it('pending buffer never leaks into a different game instance', () => {
    ...
    const playerA = makePlayer({ position: { x: 10.001, y: 10 }, noise: 0, detection: 80, depthLayer: 'Deep' })
...
describe('aiSystem LKP maintenance (F5)', () => {
  ...
  it('LKP error grows when the player maneuvers out of sensor range (drift, cap 1.5 km)', () => {
    const escort = makeEnemy({ position: { x: 10, y: 10 }, aiState: 'SEARCHING', lkp: { x: 12, y: 12, errorKm: 0 } })
