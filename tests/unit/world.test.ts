...
import { activeWeather, createWorldSystem, initWorld, weatherModifiersFor } from '../../src/world/world'
...
describe('currents (visual-only, FR-16)', () => {
  ...
  it('identical models give identical currents (pure function of the model)', () => {
    ...
    for (const [x, y] of [[0, 0], [7.3, 9.1], [29.99, 0.01], [15, 15]] as const) {
  ...
  it('vectors stay within the visual speed band and are bounded anywhere', () => {
    ...
    for (let gy = 0; gy < field.size; gy++) {
      for (let gx = 0; gx < field.size; gx++) {
        check(((gx + 0.5) / field.size) * field.extentKm, ((gy + 0.5) / field.size) * field.extentKm)
      }
    }
...
describe('src/world source hygiene', () => {
  ...
  it('contains no Math.random and no browser/DOM API references', () => {
    for (const [name, src] of sources) {
      ...
      expect(src.match(/\bwindow\b|\bdocument\b|localStorage|AudioContext|performance\.now|Date\.now/), `${name} must stay @pure`).toBeNull()
    }
  })
})
