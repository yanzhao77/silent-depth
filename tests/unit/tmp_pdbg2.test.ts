...
describe('pdbg2', () => {
  it('trace', () => {
    ...
    for (; t < 400; t++) {
      ...
      if (
        t % 10 === 0 ||
        (snap.periscope.state !== 'SUBMERGED' && snap.periscope.state !== 'SURFACING')
      ) {
        console.log(
          't=' + t,
          'state=' + snap.periscope.state,
          'prog=' + snap.periscope.progress.toFixed(2),
          'depth=' + snap.playerSub.depthLayer,
          'tgt=' + snap.playerSub.targetDepthLayer,
          'transT=' + snap.playerSub.depthTransitionT,
        );
      }
      if (snap.periscope.state === 'RAISED') {
        console.log('RAISED at t=' + t);
        break;
      }
    }
