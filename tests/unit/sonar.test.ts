...
import { PASSIVE_OBS_INTERVAL_S } from '../../src/sonar/passive';
...
describe('active ping (GAME_DESIGN §5.1)', () => {
  it('detects an enemy within 10 km with error-bounded bearing and range', () => {
    ...
    const trueRange = 10;
    expect(angularDiffDeg(c.bearingDeg, trueBearing)).toBeLessThanOrEqual(
      BALANCE.sonar.active.bearingErrorDeg + 1e-9,
    );
...
describe('passive listen (GAME_DESIGN §5.2)', () => {
  ...
  it('bearing error converges from ±3° to ±1° over 30 s of tracking', () => {
    ...
    const trueBearing = compassBearing(ctx.player.position, enemy.position);
    expect(angularDiffDeg(c2.bearingDeg, trueBearing)).toBeLessThanOrEqual(
      c2.bearingErrorDeg + 1e-9,
    );
  });
...
describe('classification voting (GAME_DESIGN §5.5)', () => {
  it('votes Merchant for a slow high-noise Strong echo', () => {
    const vote = voteClassification({ speedEstimateKt: 9, noise: 76, signal: 'Strong' }, BALANCE);
  ...
  it('votes Destroyer for a fast medium-noise Medium echo', () => {
    const vote = voteClassification({ speedEstimateKt: 25, noise: 65, signal: 'Medium' }, BALANCE);
