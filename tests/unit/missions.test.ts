...
function makeContact(state: ContactState, classification: ContactType, trueShipId: string | null = null): Contact {
  contactSeq += 1
  return {
...
function makeEnemy(id: string, shipClass: ShipClass, x: number, y: number, hull: number): EnemyShip {
  return {
    id,
    shipClass,
    position: { x, y },
...
function makePlayer(mission: MissionDef, balance: BalanceConfig, overrides: Partial<SubmarineState> = {}): SubmarineState {
  return {
...
function zeroScore(): ScoreParts {
  return { objective: 0, damage: 0, stealth: 0, torpedoEfficiency: 0, time: 0, survival: 0, total: 0, grade: 'Failed' }
}
  ...
  it('matches balance for torpedoes, parMinutes, seeds, weather, escorts and unlock', () => {
    for (const bal of balance.missions) {
      ...
      const balClasses = new Set(
        Array.isArray(bal.escortType) ? bal.escortType : bal.escortType !== undefined ? [bal.escortType] : [],
      )
...
describe('getMissionDef', () => {
  ...
  it('produces a valid MissionDef for every id (table fields + §9.2 defaults)', () => {
    for (const id of ids) {
      ...
      expect(def.visibilityKm).toBe(balance.weather[def.weather.split('->')[0] as keyof typeof balance.weather]!.visibilityKm)
    }
  })
  ...
  it('spawn compositions match the table (merchants + escorts)', () => {
    ...
    for (const [id, [enemies, escorts]] of Object.entries(byId)) {
      ...
      for (const [cls, n] of Object.entries(enemies)) expect(counts.get(cls), `${id} ${cls}`).toBe(n)
  ...
  it('passes the engine validateMissionDef contract for every id', () => {
    for (const id of ids) {
      ...
      expect(Number.isInteger(def.playerStart.x) || typeof def.playerStart.x === 'number').toBe(true)
      for (const s of def.spawns) {
        expect(s.type in balance.enemyAI.shipTypes, `${id} spawn type ${s.type}`).toBe(true)
        expect(Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.headingDeg)).toBe(true)
      }
...
describe('generator', () => {
  ...
  it('convoy merchants sit at their 2×2 formation slots (≥ 400 m apart)', () => {
    for (const id of ['M03', 'M04', 'M05']) {
      ...
      expect(minPair).toBeGreaterThanOrEqual(balance.enemyAI.escort.formation.rowSpacingM / 1000 - 1e-9)
    }
  })
  ...
  it('merchant groups of different classes stay ≥ 2 km apart (§9.2 rule)', () => {
    ...
    expect(validateSpawns(def2, input, balance)).toMatch(/merchant groups closer than 2 km|spawns too close/)
  })
  ...
  it('retries with seed++ up to maxGenRetries then throws for unsatisfiable constraints', () => {
    ...
    expect(() => generateMission(baseInput(), 1003, bad)).toThrow(new RegExp(`${attempts} attempts`))
  })
...
describe('objectives — sink objectives & defeat', () => {
  ...
  it('updates score as running totals each tick', () => {
    ...
    ctx.enemies.filter((e) => e.shipClass === 'Cargo').slice(0, 2).forEach((e) => (e.hull = 0))
    objectivesTick(ctx)
