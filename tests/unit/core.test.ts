...
describe('engine lifecycle', () => {
  ...
  it('never throws on absurd inputs — clamps are applied deterministically', () => {
    const handle = createGame(makeMission(), 1001);
    step(handle, FIXED_DT, IDLE_INPUT);
    for (let i = 0; i < 39; i++) step(handle, FIXED_DT, IDLE_INPUT);
    const wild = {
      ...IDLE_INPUT,
      throttle: 9999,
      rudder: 12345,
  ...
  it('createGame validates the mission definition', () => {
    expect(() =>
      createGame(
        { ...makeMission(), spawns: [{ type: 'Submarine', x: 0, y: 0, headingDeg: 0 }] },
        1,
      ),
...
describe('engine determinism', () => {
  ...
  it('different inputs produce different snapshots (fireRejected surfaces)', () => {
    ...
    expect(
      withFire[withFire.length - 1]!.eventLog.some((e) => e.type === 'torpedo.fireRejected'),
    ).toBe(true);
