  ...
  colorAt(u: number): [number, number, number, number] {
    ...
    for (let i = 0; i < s.length - 1; i++) {
      ...
      if (u <= b.t) {
        ...
        return [
          a.r + (b.r - a.r) * f, a.g + (b.g - a.g) * f, a.b + (b.b - a.b) * f, a.a + (b.a - a.a) * f,
        ]
      }
    }
...
export class SoftCtx {
  ...
  save(): void { this.stack.push({ t: [...this.t], alpha: this.globalAlpha, dash: [...this.dash] }) }
  restore(): void {
    const s = this.stack.pop()
    if (s) { this.t = s.t; this.globalAlpha = s.alpha; this.dash = s.dash }
  }
  ...
  setTransform(a: number, b: number, c: number, d: number, e: number, f: number): void { this.t = [a, b, c, d, e, f] }
  setLineDash(d: number[]): void { this.dash = d }

  beginPath(): void { this.path = []; this.subpaths = [] }
  moveTo(x: number, y: number): void { this.path.push(x, y); this.subpaths.push(this.path.length) }
  lineTo(x: number, y: number): void { this.path.push(x, y) }
  closePath(): void {
    const p = this.path
    const start = this.subpaths.length > 1 ? this.subpaths[this.subpaths.length - 2]! - 2 : 0
    if (p.length >= start + 4) { p.push(p[start]!, p[start + 1]!) }
  }
  ...
  arcTo(x1: number, y1: number, x2: number, y2: number, r: number): void {
    ...
    if (d1 < 1e-9 || d2 < 1e-9) { this.lineTo(x1, y1); return }
    const ux = (x1 - x0) / d1, uy = (y1 - y0) / d1
    const vx = (x2 - x1) / d2, vy = (y2 - y1) / d2
    ...
    const cx = x1 - ux * tlen, cy = y1 - uy * tlen
    const ex = x1 + vx * tlen, ey = y1 + vy * tlen
    // arc center
    const n1x = -uy, n1y = ux
    const midX = (cx + ex) / 2, midY = (cy + ey) / 2
    const dist = Math.hypot(midX - x1, midY - y1)
    const h = Math.sqrt(Math.max(0, r * r - tlen * tlen))
    let ox = x1 + n1x * (tlen * (r - dist) / r) // approx
    void ox
    const ccx = midX + n1x * h, ccy = midY + n1y * h
    void ccx; void ccy
    ...
    for (let i = 1; i <= steps; i++) {
      const a = ccw ? sAng + ((eAng - sAng + (eAng < sAng ? Math.PI * 2 : 0)) * i) / steps : sAng - ((sAng - eAng + (sAng < eAng ? Math.PI * 2 : 0)) * i) / steps
      const aa = ccw ? a : a
  ...
  ellipse(x: number, y: number, rx: number, ry: number, rot: number, start: number, end: number, ccw = false): void {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      p.push(x + px * Math.cos(rot) - py * Math.sin(rot), y + px * Math.sin(rot) + py * Math.cos(rot))
    }
  }
  ...
  fill(): void {
    ...
    for (let y = y0; y <= y1; y++) {
      ...
      for (let i = 0; i + 1 < hits.length; i += 2) {
        const ha = Math.max(0, Math.ceil(hits[i]!)), hb = Math.min(this.canvas.width - 1, Math.floor(hits[i + 1]!))
  ...
  stroke(): void {
    ...
    const x0 = Math.max(0, Math.floor(minX - r)), x1 = Math.min(this.canvas.width - 1, Math.ceil(maxX + r))
    const y0 = Math.max(0, Math.floor(minY - r)), y1 = Math.min(this.canvas.height - 1, Math.ceil(maxY + r))
    for (let y = y0; y <= y1; y++) {
      for (let x = x0; x <= x1; x++) {
        let best = Infinity
        for (const [ax, ay, bx, by] of screenSegs) {
  ...
  clearRect(x: number, y: number, w: number, h: number): void {
    ...
    const xb = Math.min(this.canvas.width, Math.ceil(x0 + w)), yb = Math.min(this.canvas.height, Math.ceil(y0 + h))
    for (let yy = ya; yy < yb; yy++) {
      for (let xx = xa; xx < xb; xx++) {
        const i = (yy * this.canvas.width + xx) * 4
        this.canvas.data[i] = 0; this.canvas.data[i + 1] = 0; this.canvas.data[i + 2] = 0; this.canvas.data[i + 3] = 0
      }
    }
  }
  ...
  drawImage(img: SoftCanvas, ...args: number[]): void {
    ...
    if (args.length === 4) {
      ...
      dx = args[0]!; dy = args[1]!; dw = img.width; dh = img.height
    } else {
      return
    }
    if (dw <= 0 || dh <= 0) return
  ...
  createRadialGradient(x0: number, y0: number, r0: number, x1: number, y1: number, r1: number): SoftGradient {
  ...
  getImageData(): ImageData {
    return { width: this.canvas.width, height: this.canvas.height, data: this.canvas.data } as ImageData
  }
}
