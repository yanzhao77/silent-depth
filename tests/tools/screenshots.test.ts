...
function runTo(
  handle: ReturnType<typeof createGame>,
  ticks: number,
  brain?: (s: GameSnapshot) => PlayerInputs,
): GameSnapshot {
  let snap = step(handle, FIXED_DT, IDLE);
  for (let i = 0; i < ticks; i++) {
    const inputs = brain ? brain(snap) : IDLE;
    if (inputs.ping) void inputs.ping;
...
function renderAndSave(
  ...
  frame?: { ping?: boolean; torpedo?: boolean },
): string {
  ...
  if (frame?.ping)
    particles.spawnPing(snapshot.playerSub.position.x, snapshot.playerSub.position.y);
  if (frame?.torpedo) {
    for (const t of snapshot.torpedoes)
      if (t.state === 'RUNNING') particles.spawnWake(t.position.x, t.position.y, t.headingDeg);
  }
...
describe('headless screenshot generator (README previews)', () => {
  ...
  it('m02-ambush: player + tanker contact + ping ring', () => {
    ...
    const snap = runTo(h, 60, (s) =>
      s.playerSub.pingCooldown <= 0 ? { ...IDLE, ping: true } : IDLE,
    );
  ...
  it('m02-torpedo: torpedo in flight with wake', () => {
    ...
    const camera = createCamera({
      zoom: 6,
      center: { x: snap.playerSub.position.x + 1.2, y: snap.playerSub.position.y },
      viewport: { width: W, height: H },
    });
