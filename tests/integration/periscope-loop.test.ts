...
function confidencePenaltyAt(confidence: number, balance: BalanceConfig): number {
  const pts = Object.entries(balance.hitProbability.confidencePen)
    .map(([key, value]) => [Number.parseInt(key.replace(/^(ge|lt)/, ''), 10), value] as [number, number])
    .sort((a, b) => a[0] - b[0])
...
export function playPeriscopeLoop(
  opts: { seed?: number; standoffKm?: number; lowerMode?: 'edge' | 'emergency'; lowerAfterS?: number } = {},
): PeriscopeLoopTrace {
  ...
  trace.neverImmediatelyRaised = snap.periscope.state !== 'RAISED' && snap.periscope.state !== 'RAISING'
  ...
  const detAtRaising = snap.playerSub.detection
  snap = step(handle, FIXED_DT, PERI)
  ...
  expect(Math.abs(observed.headingEstimateDeg! - normalizeDeg(90 - tanker.headingDeg))).toBeLessThan(2)
  ...
  if (lowerMode === 'emergency') {
    ...
    expect(trace.emergencyBatteryDelta).toBeGreaterThanOrEqual(BALANCE.battery.emergencyDiveCostPercent)
    expect(trace.emergencyBatteryDelta).toBeLessThanOrEqual(BALANCE.battery.emergencyDiveCostPercent + 0.01)
  ...
  for (let i = 0; i < 600 && snap.state === 'MISSION_RUNNING'; i++) snap = step(handle, FIXED_DT, PERI)
  trace.final = snap
  ...
  trace.lifecycleEvents = events.filter((t) => t.startsWith('periscope') || ['torpedo.fired'].includes(t))
