...
# Blender localises default node names to the UI language ("原理化 BSDF" under
# Chinese), but the material setup below looks nodes up by their English names.
# Force English so the build behaves the same on every machine.
...
def metadata(stats,checks):
    ...
    (ROOT/"Documentation"/(ID+"_README.md")).write_text(
        "# RU_SSN_Akula\n\n状态：VALIDATING。\n\n"
            ...
            "推进器单独导出为FBX/"+ID+"_PROP.fbx，原点落在桨轴上，便于引擎侧让桨叶旋转；"
            "艇体LOD不含桨叶几何。\n\n"
