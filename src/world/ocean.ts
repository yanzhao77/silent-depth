...
function sampleGridScalar(cells: number[], size: number, u: number, v: number, wrap: boolean): number {
...
function generateCurrentField(ripple: ValueNoiseGrid, size: number, extentKm: number): CurrentField {
...
function buildDepthGradient(): Record<DepthLayer, string> {
  ...
  for (let i = 0; i < n; i++) {
    ...
    out[DEPTH_LAYER_ORDER[i]!] = interpolateHex(OCEAN_PALETTE.shallow, OCEAN_PALETTE.base, i / (n - 1))
  }
  return out
}
...
export function generateOcean(seed: number, balance: BalanceConfig): OceanModel {
  ...
  const currentField = generateCurrentField(rippleNoise, CURRENT_GRID_SIZE, balance.world.mapSizeKm)
  return {
