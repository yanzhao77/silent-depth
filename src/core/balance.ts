...
import type { AiState, DepthLayer, ScoreGrade, SpeedBand, TorpedoState, WeatherKind } from './types'
...
export interface SonarConfig {
  ...
  classification: {
    ...
    types: Record<string, { speedRangeKt: [number, number]; noiseRange: [number, number]; surfaceOnly: boolean }>
  }
}
...
export interface EnemyAIConfig {
  ...
  searchPatterns: {
    circular: { radiusStartKm: number; radiusMaxKm: number; radiusStepPerLapM: number; speedKt: number }
...
function assertDeepNumber(cfg: Record<string, unknown>, path: string[]): void {
  ...
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    throw new BalanceConfigError(`loadBalance: expected a finite number at "config/balance.json → ${path.join('.')}"`)
  }
}
...
function assertDeepObject(cfg: Record<string, unknown>, path: string[]): void {
  ...
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new BalanceConfigError(`loadBalance: expected an object at "config/balance.json → ${path.join('.')}"`)
  }
}
...
export function loadBalance(source: unknown = balanceJson): BalanceConfig {
  ...
  for (const key of REQUIRED_TOP_LEVEL_KEYS) {
    if (!(key in cfg)) {
      throw new BalanceConfigError(`loadBalance: missing required key "${key}" in config/balance.json`)
    }
  }
  ...
  if (!Array.isArray(missions) || missions.length < 5) {
    throw new BalanceConfigError('loadBalance: "missions" must be an array with at least 5 mission definitions')
  }
