...
function validateMissionDef(def: MissionDef): void {
  ...
  if (
    def.objective === null ||
    typeof def.objective !== 'object' ||
    typeof def.objective.kind !== 'string'
  ) {
    fail('"objective.kind" must be a string');
  }
  if (
    typeof def.playerStart?.x !== 'number' ||
    typeof def.playerStart?.y !== 'number' ||
    typeof def.playerStart?.headingDeg !== 'number'
  ) {
  ...
  for (const spawn of def.spawns) {
    ...
    if (
      ...
      typeof spawn.headingDeg !== 'number'
    ) {
      fail('each spawn must have numeric x/y/headingDeg');
    }
  }
  if (!Number.isInteger(def.torpedoCount) || def.torpedoCount <= 0)
    fail('"torpedoCount" must be a positive integer');
  if (
    typeof def.batteryStart !== 'number' ||
    def.batteryStart < 0 ||
    def.batteryStart > balance.battery.capacity
  ) {
    fail(`"batteryStart" must be within [0, ${balance.battery.capacity}]`);
  }
  if (
    def.briefingSeconds !== undefined &&
    (typeof def.briefingSeconds !== 'number' || def.briefingSeconds < 0)
  ) {
...
function initialSubmarineState(mission: MissionDef, balance: BalanceConfig): SubmarineState {
  const tubes: SubmarineState['torpedoTubes'] = Array.from(
    { length: mission.torpedoCount },
    (_, i) => ({
...
export function step(handle: GameHandle, dtSeconds: number, inputs: PlayerInputs): GameSnapshot {
  ...
  if (!ctx.skip) {
    ...
    const { inputs: normalized, rejections } = normalizeInputs(
      inputs,
      rt.balance,
      contactIdSet(rt),
    );
    ...
    for (const rejection of rejections) {
      rt.bus.emit('torpedo.fireRejected', {
        reason: rejection.reason,
        contactId: rejection.contactId,
      });
    }
