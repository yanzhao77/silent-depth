...
export function createMenus(
  ...
  function renderMainMenu(): void {
    ...
    root.append(
      el('div', { className: 'menu-panel' }, [
        ...
        menuButton(
          tt('menu.play'),
          () => deps.onPlay(lastUnlocked),
          'primary',
          tt('menu.quickStart', { id: lastUnlocked }),
        ),
        ...
        el('div', {
          className: 'menu-meta',
          text: tt('menu.nextMission', { id: lastUnlocked, best }),
        }),
      ...
      return el(
        ...
        [
          ...
          el('span', {
            ...
            text: `${spec.id} ${tt(`mission.${spec.id}.name`)}`,
          }),
          el('span', {
            className: 'mission-meta',
            text: tt('missions.rowMeta', { diff: spec.difficulty, par: spec.parMinutes, best }),
          }),
        ],
      );
    });
  ...
  function renderBriefing(ctx: MenuContext): void {
    ...
    root.append(
      el('div', { className: 'menu-panel' }, [
        ...
        el('h1', {
          className: 'menu-title small',
          text: `${def.id} ${tt(`mission.${def.id}.name`)}`,
        }),
        el('div', { className: 'briefing-section' }, [
          ...
          ...subgoals.map((g) =>
            el('div', {
              ...
              text: `— ${tt(`mission.${def.id}.obj.${g.id}`)}`,
            }),
          ),
        ]),
        el('div', { className: 'briefing-section' }, [
          ...
          el('div', {
            className: 'briefing-line',
            text: tt('briefing.weather', {
              w: weatherChain(def.weather, tt),
              vis: def.visibilityKm,
            }),
          }),
          el('div', {
            className: 'briefing-line',
            text: tt('briefing.torpedoes', {
              n: def.torpedoCount,
              pct: Math.round(def.batteryStart),
              par: formatPar(def.parTimeS),
            }),
          }),
        ]),
        el('div', { className: 'menu-spacer' }),
        el('div', {
          className: 'briefing-count',
          text: tt('briefing.countdown', { s: remaining }),
        }),
      ]),
    );
  }
  ...
  function renderLoading(): void {
    ...
    root.append(
      el('div', { className: 'menu-panel' }, [
        el('div', { className: 'menu-subtitle', text: tt('app.loading') }),
      ]),
    );
  }
  ...
  function renderVictoryDefeat(state: 'VICTORY' | 'DEFEAT'): void {
    ...
    root.append(
      el('div', { className: 'menu-panel' }, [
        el('h1', {
          className: `menu-title ${state === 'VICTORY' ? 'victory' : 'defeat'}`,
          text: state === 'VICTORY' ? tt('outcome.victory') : tt('outcome.defeat'),
        }),
  ...
  function renderResult(ctx: MenuContext): void {
    ...
    root.append(
      el('div', { className: 'menu-panel result' }, [
        ...
        el('h1', {
          className: `menu-title ${gradeClass(score.grade)}`,
          text: tt(`result.grade.${score.grade}`),
        }),
        ...
        el('div', {
          ...
          text: tt('result.stats', {
            hit: stats.torpedoesHit,
            fired: stats.torpedoesFired,
            left: stats.torpedoesRemaining,
            d: Math.round(stats.peakDetection),
            t: formatPar(stats.elapsedS),
          }),
        }),
...
function convoyReport(def: MissionDef, tt: Translator): string {
  ...
  const merchants =
    (counts.get('Merchant') ?? 0) + (counts.get('Cargo') ?? 0) + (counts.get('Tanker') ?? 0);
...
function selectParticles(
  tt: Translator,
  current: SaveSettings['video']['particles'],
  onChange: (v: SaveSettings['video']['particles']) => void,
): HTMLElement {
  const sel = el('select', {
    onchange: (e) =>
      onChange((e.target as HTMLSelectElement).value as SaveSettings['video']['particles']),
  });
