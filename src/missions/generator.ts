...
export function slotForMerchantIndex(
  index: number,
...
function buildMission(input: GeneratorInput, seed: number, balance: BalanceConfig): MissionDef {
  ...
  const merchantClasses = Object.keys(input.enemies).filter(
    (c) => (input.enemies[c] ?? 0) > 0,
  ) as ShipClass[];
  ...
  const forwardExtentKm =
    w.merchantSpawnSpreadKm +
    geo.rowSpacingM / 1000 +
    w.playerSpawnMinDistKm +
    PLAYER_AHEAD_RANGE_KM;
  const alongBandLow =
    forwardSign === 1 ? backExtentKm + MAP_EDGE_MARGIN_KM : forwardExtentKm + MAP_EDGE_MARGIN_KM;
  const alongBandHigh =
    forwardSign === 1
      ? w.mapSizeKm - forwardExtentKm - MAP_EDGE_MARGIN_KM
      : w.mapSizeKm - backExtentKm - MAP_EDGE_MARGIN_KM;
  // Empty band (e.g. an overridden playerSpawnMinDistKm) → place at the map
  // centre and let validation fail + retry naturally.
  const cAlong =
    alongBandHigh < alongBandLow ? w.mapSizeKm / 2 : rng.range(alongBandLow, alongBandHigh);
  const cPerp = rng.range(
    MAP_EDGE_MARGIN_KM + PLAYER_LATERAL_KM,
    w.mapSizeKm - MAP_EDGE_MARGIN_KM - PLAYER_LATERAL_KM,
  );
  ...
  const visibilityKm =
    balance.weather[firstKind]?.visibilityKm ?? balance.weather.Clear.visibilityKm;
...
export function validateSpawns(
  ...
  const inMap = (p: { x: number; y: number }, marginKm: number): boolean =>
    p.x >= marginKm &&
    p.x <= w.mapSizeKm - marginKm &&
    p.y >= marginKm &&
    p.y <= w.mapSizeKm - marginKm;
  ...
  for (let i = 0; i < def.spawns.length; i++) {
    for (let j = i + 1; j < def.spawns.length; j++) {
      if (distKm(def.spawns[i]!, def.spawns[j]!) < MIN_SPAWN_SEPARATION_KM)
        return 'spawns too close';
    }
  }
