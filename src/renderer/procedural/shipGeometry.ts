...
const DETAIL_BY_LOD: Readonly<Record<ShipLodLevel, ShipDetail>> = {
  0: {
    ...
    railSections: 3,
    windowCount: 4,
    includeNavigationLights: true,
    includeWeapons: true,
    includeSecondaryDetails: true,
  },
  1: {
    cargoRows: 6,
    tankCount: 4,
  ...
  2: {
    ...
    railSections: 0,
    windowCount: 1,
    includeNavigationLights: false,
    includeWeapons: true,
    includeSecondaryDetails: false,
  },
  3: {
    cargoRows: 1,
    tankCount: 1,
...
function createMaterials(shipClass: string): ShipMaterials {
  return {
    hull: new THREE.MeshStandardMaterial({
      color: HULL_COLORS[shipClass] ?? HULL_COLORS.Merchant,
      roughness: 0.57,
      metalness: 0.48,
    }),
    superstructure: new THREE.MeshStandardMaterial({
      color: SUPERSTRUCTURE_COLORS[shipClass] ?? SUPERSTRUCTURE_COLORS.Merchant,
      roughness: 0.48,
      metalness: 0.42,
    }),
    ...
    portLight: new THREE.MeshStandardMaterial({
      color: 0x4b100e,
      roughness: 0.36,
      metalness: 0.12,
      emissive: 0x7c0b06,
    ...
    starboardLight: new THREE.MeshStandardMaterial({
      color: 0x073128,
      roughness: 0.36,
      metalness: 0.12,
      emissive: 0x067451,
    ...
    sternLight: new THREE.MeshStandardMaterial({
      color: 0x4c461c,
      roughness: 0.38,
      metalness: 0.11,
      emissive: 0x7d6b1b,
...
function addBridge(
  ...
  const bridge = new THREE.Mesh(
    new THREE.BoxGeometry(
      dims.length * 0.115,
      dims.freeboard * heightFactor,
      dims.beam * widthFactor,
    ),
...
function addDeckRails(
  ...
  for (let i = 0; i < detail.railSections; i++) {
    const x = -dims.length * 0.12 + i * dims.length * 0.1;
    for (const side of [-1, 1]) {
      const post = new THREE.Mesh(
        new THREE.CylinderGeometry(0.00012, 0.00012, dims.freeboard * 1.15, 5),
        materials.metal,
      );
    ...
    for (const side of [-1, 1]) {
      const rail = new THREE.Mesh(
        new THREE.CylinderGeometry(0.0001, 0.0001, dims.length * 0.085, 5),
        materials.metal,
      );
...
function addMerchant(
  ...
  addBridge(group, dims, materials, -dims.length * 0.32, 0.56, 2.3, detail);
  const mast = new THREE.Mesh(
    new THREE.CylinderGeometry(0.0007, 0.0009, dims.freeboard * 5.3, 6),
    materials.metal,
  );
  ...
  const craneArm = new THREE.Mesh(
    new THREE.BoxGeometry(dims.length * 0.1, 0.00055, 0.00055),
    materials.metal,
  );
  ...
  for (let i = 0; i < detail.hatchCount; i++) {
    const hatch = new THREE.Mesh(
      new THREE.BoxGeometry(dims.length * 0.13, 0.00055, dims.beam * 0.52),
      materials.deck,
    );
  ...
  if (detail.includeSecondaryDetails) {
    const rustPatch = new THREE.Mesh(
      new THREE.BoxGeometry(dims.length * 0.1, dims.freeboard * 0.2, 0.0002),
      materials.rust,
    );
...
function addCargo(
  ...
  const mutedContainerMats = [
    ...
    new THREE.MeshStandardMaterial({ color: 0x75684a, roughness: 0.8, metalness: 0.16 }),
  ];
  const containerLength = dims.length * 0.09;
  const containerHeight = dims.freeboard * 1.1;
  ...
  for (let row = 0; row < detail.cargoRows; row++) {
    ...
    const x = dims.length * 0.2 - row * containerLength * 0.94;
    const container = new THREE.Mesh(
      new THREE.BoxGeometry(containerLength, containerHeight, containerDepth),
      material,
    );
    ...
    if (detail.includeSecondaryDetails && row % 2 === 0) {
      const upper = new THREE.Mesh(
        new THREE.BoxGeometry(
          containerLength * 0.94,
          containerHeight * 0.72,
          containerDepth * 0.94,
        ),
        material,
      );
  ...
  const funnel = new THREE.Mesh(
    new THREE.CylinderGeometry(0.002, 0.0025, dims.freeboard * 2.25, 8),
    materials.deck,
  );
...
function addTanker(
  ...
  for (let i = 0; i < detail.tankCount; i++) {
    const dome = new THREE.Mesh(
      new THREE.CylinderGeometry(tankRadius, tankRadius, tankLength, 14),
      materials.superstructure,
    );
  ...
  for (const side of [-1, 1]) {
    const pipe = new THREE.Mesh(
      new THREE.CylinderGeometry(0.00042, 0.00042, dims.length * 0.66, 6),
      materials.pipe,
    );
  ...
  addBridge(group, dims, materials, -dims.length * 0.38, 0.57, 3.6, detail);
  const stack = new THREE.Mesh(
    new THREE.CylinderGeometry(0.002, 0.0028, dims.freeboard * 2.6, 8),
    materials.deck,
  );
  ...
  if (detail.includeSecondaryDetails) {
    const manifold = new THREE.Mesh(
      new THREE.BoxGeometry(dims.length * 0.1, dims.freeboard * 0.46, dims.beam * 0.36),
      materials.rust,
    );
...
function addGun(
  ...
  const turret = new THREE.Mesh(
    new THREE.CylinderGeometry(dims.beam * 0.18, dims.beam * 0.22, dims.freeboard * 0.65, 12),
    materials.weapon,
  );
  turret.position.set(x, y, 0);
  add(group, turret, `${name}-turret`, true);
  const barrel = new THREE.Mesh(
    new THREE.CylinderGeometry(0.00068, 0.00068, dims.length * 0.095, 6),
    materials.weapon,
  );
...
function addDestroyer(
  ...
  addBridge(group, dims, materials, dims.length * 0.08, 0.54, 4.1, detail);
  if (detail.includeWeapons) {
    addGun(
      ...
      dims,
      materials,
      'destroyer-forward-gun',
    );
    addGun(group, -dims.length * 0.26, dims.freeboard * 1.3, dims, materials, 'destroyer-aft-gun');
  }
  const mast = new THREE.Mesh(
    new THREE.CylinderGeometry(0.00056, 0.00068, dims.freeboard * 7.4, 6),
    materials.metal,
  );
  ...
  if (detail.includeSecondaryDetails) {
    const radar = new THREE.Mesh(
      new THREE.BoxGeometry(0.00045, 0.00042, dims.beam * 0.72),
      materials.metal,
    );
    ...
    for (const side of [-1, 1]) {
      const rack = new THREE.Mesh(
        new THREE.BoxGeometry(dims.length * 0.09, dims.freeboard * 0.5, dims.beam * 0.16),
        materials.weapon,
      );
      rack.position.set(-dims.length * 0.08, dims.freeboard * 1.2, side * dims.beam * 0.31);
      add(group, rack, 'destroyer-torpedo-rack');
    }
  }
  const funnel = new THREE.Mesh(
    new THREE.CylinderGeometry(0.0019, 0.00245, dims.freeboard * 2.2, 8),
    materials.deck,
  );
  funnel.position.set(-dims.length * 0.1, dims.freeboard * 2.65, 0);
...
function addFrigate(
  ...
  const mast = new THREE.Mesh(
    new THREE.CylinderGeometry(0.00075, 0.00092, dims.freeboard * 7.5, 8),
    materials.metal,
  );
  ...
  const array = new THREE.Mesh(
    new THREE.BoxGeometry(dims.length * 0.055, dims.freeboard * 1.5, 0.00065),
    materials.weapon,
  );
  ...
  const hangar = new THREE.Mesh(
    new THREE.BoxGeometry(dims.length * 0.13, dims.freeboard * 2.1, dims.beam * 0.52),
    materials.superstructure,
  );
  hangar.position.set(-dims.length * 0.2, dims.freeboard * 2.1, 0);
  add(group, hangar, 'frigate-hangar', true);
  const flightDeck = new THREE.Mesh(
    new THREE.BoxGeometry(dims.length * 0.24, 0.0005, dims.beam * 0.66),
    materials.deck,
  );
  flightDeck.position.set(-dims.length * 0.34, dims.freeboard + 0.0003, 0);
  add(group, flightDeck, 'frigate-flight-deck');
  if (detail.includeSecondaryDetails) {
    const landingMark = new THREE.Mesh(
      new THREE.RingGeometry(dims.beam * 0.15, dims.beam * 0.17, 16),
      materials.metal,
    );
  ...
  const funnel = new THREE.Mesh(
    new THREE.CylinderGeometry(0.00175, 0.00225, dims.freeboard * 1.95, 8),
    materials.deck,
  );
...
export function createShipGeometry(shipClass: string, lod: ShipLodLevel = 1): THREE.Group {
  ...
  const stripe = new THREE.Mesh(
    new THREE.BoxGeometry(dims.length * 0.95, 0.0007, dims.beam * 1.02),
    materials.waterline,
  );
  stripe.position.y = dims.draft * 0.5;
  add(group, stripe, 'waterline-marking');
  const deck = new THREE.Mesh(
    new THREE.BoxGeometry(dims.length * 0.82, 0.0006, dims.beam * 0.8),
    materials.deck,
  );
  ...
  if ((shipClass === 'Tanker' || shipClass === 'Cargo') && lod < 3) {
    const bulb = new THREE.Mesh(new THREE.SphereGeometry(dims.beam * 0.2, 10, 8), materials.hull);
    bulb.position.set(dims.length / 2 + dims.beam * 0.1, dims.draft * 0.3, 0);
  ...
  switch (shipClass) {
    case 'Cargo':
      addCargo(group, dims, materials, detail);
      break;
    case 'Tanker':
      addTanker(group, dims, materials, detail);
      break;
    case 'Destroyer':
      addDestroyer(group, dims, materials, detail);
      break;
    case 'Frigate':
      addFrigate(group, dims, materials, detail);
      break;
    default:
      addMerchant(group, dims, materials, detail);
      break;
  }
