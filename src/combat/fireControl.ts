...
export function solveFireSolution(
  ...
  if (heading !== null && speed !== null && speed > 0) {
    ...
    leadAngleDeg =
      (Math.atan2(speed * Math.sin(aobRad), speed * Math.cos(aobRad) + torpedoKt) * 180) / Math.PI;
  }
...
function speedPenalty(speedKt: number | null, balance: BalanceConfig): number {
  const pts = penPoints(balance.hitProbability.targetSpeedPen, (k) =>
    Number.parseInt(k.replace('le', ''), 10),
  );
...
function confidencePenalty(confidence: number, balance: BalanceConfig): number {
  const pts = penPoints(balance.hitProbability.confidencePen, (k) =>
    Number.parseInt(k.replace(/^(ge|lt)/, ''), 10),
  );
