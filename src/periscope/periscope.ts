  ...
  switch (p.state) {
    case 'SUBMERGED': {
      ...
      if (depthOk) {
        ...
        const reason = isDeeperThan(player.depthLayer, cfg.requiredLayer) ? 'tooDeep' : 'wrongLayer'
    ...
    case 'OBSERVING': {
      ...
      if (rateIdx >= 0 && rateIdx < cfg.exposureDetectPerSec.length) {
        player.detection = clamp(player.detection + cfg.exposureDetectPerSec[rateIdx]! * ctx.dt, 0, 100)
      }
      ...
      // --- lock (edge) ---
      if (ctx.lockEdge && p.observingContactId !== null && p.lockedContactId !== p.observingContactId) {
        const observed = ctx.contacts.find((c) => c.id === p.observingContactId)
        const rangeKm =
          observed !== undefined
            ? distKm(player.position, ctx.enemies.find((e) => e.id === observed.trueShipId)?.position ?? player.position)
            : Infinity
...
function enterLowering(ctx: SystemContext, rt: PeriscopeRuntime, p: PeriscopePublicState, emergency: boolean): void {
...
function updateCanRaise(
  ...
  if (p.state === 'SUBMERGED') {
    if (depthOk) {
      ...
      p.canRaise = false
      p.cannotRaiseReason = isDeeperThan(player.depthLayer, requiredLayer) ? 'tooDeep' : 'wrongLayer'
    }
  } else {
    p.canRaise = false
...
function emitVisualEvents(ctx: SystemContext, rt: PeriscopeRuntime, contact: Contact): void {
  if (!rt.visualContactEmitted.has(contact.id)) {
    ...
    ctx.bus.emit('periscope.visualContact', { contactId: contact.id, classification: contact.classification })
  }
  if (!rt.classifiedEmitted.has(contact.id)) {
    rt.classifiedEmitted.add(contact.id)
    ctx.bus.emit('periscope.classified', { contactId: contact.id, classification: contact.classification })
  }
}
