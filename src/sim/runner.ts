...
import type { EventEntry, GameSnapshot, MissionDef, PlayerInputs, ScoreParts, ShipClass } from '../core/types'
...
export function runScripted(
  ...
  const actions: PlaytestActions = { pings: 0, fireInputs: 0, movingTicks: 0, turningTicks: 0, fireRejections: 0 }
  let ticks = 0
  ...
  const stats: PlaytestStats = final
    ? {
        ...
        damageDealt: final.enemies.reduce((sum, e) => sum + Math.max(0, (initialHulls.get(e.id) ?? e.hull) - e.hull), 0),
  ...
  return {
    ...
    actions: {
      ...actions,
      fireRejections: (final?.eventLog ?? []).filter((e) => e.type === 'torpedo.fireRejected').length,
    },
    stats,
    score: final?.score ?? {
      objective: 0,
...
export const STRATEGIES: Record<StrategyId, StrategyMeta> = {
  'ping-until-track': {
    ...
    description: 'Cruise toward the reported merchant position (briefing knowledge), ping inside 10 km until the contact reaches TRACKED; no torpedoes (M01 has no sink requirement).',
  },
  'stationary-ambush': {
    id: 'stationary-ambush',
    label: 'Stationary ambush (M02, PROVEN t-013/t-020)',
    description: 'Hold position STOPPED at Medium depth, sparse pings every 150 s for range, fire point-blank (≤ 1.2 km) with a fresh ping + lead-corrected fire solution; re-fire after torpedo resolution.',
  },
  'convoy-attack': {
    id: 'convoy-attack',
    label: 'Convoy attack (M03/M04 best effort)',
    description: 'Approach the convoy at CRUISE, SILENT inside 2.5 km, ping for range, fire at the nearest ranged merchant contact ≤ fire range with a fresh ping; evade (Deep + silent + decoy) when detection is hot or an escort escalates.',
  },
  'generic-hunter': {
    id: 'generic-hunter',
    label: 'Generic hunter (generated missions)',
    description: 'Silent approach (SILENT band, Medium), ping to acquire range, fire at the nearest TRACKED-or-better contact ≤ 1.5 km; evade when detection ≥ 45 or an escort escalates inside 5 km.',
  },
  'sink-and-escape': {
    id: 'sink-and-escape',
    label: 'Sink then escape (M05 best effort)',
    description: 'Phase 1: convoy-attack behavior until the first ship is sunk; Phase 2: Deep + silent running, creep away from the nearest escort to satisfy F9 (detection < 20, escorts > 3 km for 30 s).',
  },
  'determinism-check': {
    id: 'determinism-check',
    label: 'Determinism double-run (M01)',
    description: 'Re-runs the ping-until-track brain on M01/seed 1001 twice; the harness asserts the two final snapshots are byte-identical.',
  },
}
...
function nearestMerchantContact(snap: GameSnapshot): {
  ...
  const merchants = snap.enemies.filter(
    (e) => e.hull > 0 && (e.shipClass === 'Cargo' || e.shipClass === 'Merchant' || e.shipClass === 'Tanker'),
  )
  let nearest: { contactId: string; shipId: string; d: number; bearing: number; state: string } | null = null
...
function nearestEscort(snap: GameSnapshot): { id: string; d: number; bearing: number; aiState: string } | null {
  const escorts = snap.enemies.filter((e) => e.hull > 0 && (e.shipClass === 'Destroyer' || e.shipClass === 'Frigate'))
  let best: { id: string; d: number; bearing: number; aiState: string } | null = null
  for (const e of escorts) {
    const d = distKm(snap.playerSub.position, e.position)
    if (best === null || d < best.d) best = { id: e.id, d, bearing: compassBearing(snap.playerSub.position, e.position), aiState: e.aiState }
  }
  return best
}
  ...
  switch (strategy) {
    ...
    case 'determinism-check': {
      ...
      return (snap, last) => {
        ...
        const bearing = contact !== undefined ? contact.bearingDeg : compassBearing(snap.playerSub.position, target)
    ...
    case 'convoy-attack': {
      ...
      return (snap, last, st) => {
        ...
        const torpedoesSpent = snap.playerSub.torpedoTubes.every((t) => t.state !== 'LOADED' && t.state !== 'READY')
        const ev = wantEvade(snap, opts)
        const depthCharges = snap.eventLog.some((e) => e.type === 'depthCharge.dropped')
        if (ev.on || torpedoesSpent) {
          const away = ev.on ? ev.awayBearing : nearest !== null ? nearest.bearing + 180 : snap.playerSub.headingDeg
        ...
        const tubesReady = snap.playerSub.torpedoTubes.some((t) => t.state === 'LOADED' || t.state === 'READY')
        const canFire =
          nearest !== null &&
        ...
        const refresh =
          nearest !== null &&
          (nearest.d > 3 || nearest.state === 'UNKNOWN' || nearest.state === 'SUSPECTED' || nearest.state === 'CLASSIFIED') &&
          st.tick - lastPingTick >= 200 // 10 s cadence (range-fix): keeps the contact climbing to TRACKED
      // while limiting ping self-exposure
        const wantPing =
          snap.playerSub.pingCooldown <= 0 && !last.ping && snap.state === 'MISSION_RUNNING' && (acquire || refresh || fireNow)
    ...
    case 'generic-hunter': {
      ...
      return (snap, last, st) => {
        ...
        const tubesReady = snap.playerSub.torpedoTubes.some((t) => t.state === 'LOADED' || t.state === 'READY')
        const tracked =
          nearest !== null && (nearest.state === 'TRACKED' || nearest.state === 'CONFIRMED') && !torpedoRunningAt(nearest.shipId)
        const canFire =
          tracked &&
          nearest!.d <= fireRangeKm &&
          tubesReady &&
