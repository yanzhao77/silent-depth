...
function buildPlan(cfg: PlaytestConfig): SessionPlan[] {
  ...
  const gen = [
    buildGeneratedDef('GEN-01', 'Generated Merchant Pair', { Merchant: 2 }, [], 4, 'Clear', 1, 2001),
    buildGeneratedDef('GEN-02', 'Generated Cargo Pair', { Cargo: 2 }, [], 4, 'Clear->Cloudy', 1, 2002),
    buildGeneratedDef('GEN-03', 'Generated Tanker Escort', { Tanker: 2 }, ['Frigate'], 4, 'Cloudy', 2, 2003),
    buildGeneratedDef('GEN-04', 'Generated Convoy + Destroyer', { Cargo: 3 }, ['Destroyer'], 5, 'Cloudy->Storm', 2, 2004),
    buildGeneratedDef('GEN-05', 'Generated Heavy Convoy', { Cargo: 3, Tanker: 1 }, ['Destroyer', 'Frigate'], 5, 'Storm', 3, 2005),
  ]

  const plan: SessionPlan[] = [
    { session: 1, def: m01, seed: m01.seed, strategy: 'ping-until-track', brainId: 'scripted-brain-ping-until-track', maxTicks: maxTicks('ping-until-track') },
    { session: 2, def: m02, seed: m02.seed, strategy: 'stationary-ambush', brainId: 'scripted-brain-stationary-ambush', maxTicks: maxTicks('stationary-ambush') },
    { session: 3, def: m03, seed: m03.seed, strategy: 'convoy-attack', brainId: 'scripted-brain-convoy-attack', maxTicks: maxTicks('convoy-attack') },
    { session: 4, def: m04, seed: m04.seed, strategy: 'convoy-attack', brainId: 'scripted-brain-convoy-attack', maxTicks: maxTicks('convoy-attack') },
    { session: 5, def: m05, seed: m05.seed, strategy: 'sink-and-escape', brainId: 'scripted-brain-sink-and-escape', maxTicks: maxTicks('sink-and-escape') },
    ...gen.map((def, i) => ({
      session: 6 + i,
      def,
      seed: def.seed,
    ...
    { session: 11, def: m01, seed: m01.seed, strategy: 'determinism-check', brainId: 'scripted-brain-ping-until-track', maxTicks: maxTicks('determinism-check'), note: 'determinism run A' },
    { session: 12, def: m01, seed: m01.seed, strategy: 'determinism-check', brainId: 'scripted-brain-ping-until-track', maxTicks: maxTicks('determinism-check'), note: 'determinism run B' },
  ]
  return plan
}
...
function collectFindings(r: PlaytestResult, def: MissionDef): SessionFindings {
  ...
  const hasMerchants = def.spawns.some((s) => s.type === 'Cargo' || s.type === 'Merchant' || s.type === 'Tanker')
  ...
  if (stats.torpedoesFired > 0) {
    ...
    bugs.push('Fire inputs were sent but no torpedo ever launched (engine rejections / no ready tubes).')
  }
  if (actions.fireRejections > 0) {
    bugs.push(`Engine rejected ${actions.fireRejections} fire input(s) in the event tail (stale contact id or not-ready) — the brain requested an invalid launch.`)
  }
  ...
  if (hasEscorts && stats.peakDetection >= 40) {
    bugs.push(`Detection peaked at ${stats.peakDetection} — escorts escalated (SUSPICIOUS→ALERT→HUNTING) and engaged the player.`)
  }
  if (hasEscorts && stats.finalHull < 100 && outcome !== 'VICTORY') {
    bugs.push(`Escort attack damaged the player to hull ${stats.finalHull} (depth charges / deck gun).`)
    recommendations.push(
  ...
  if (stats.finalBattery < 15) {
    bugs.push(`Battery pressure: ${stats.finalBattery}% remaining — ping cost (2 %/ping) and CRUISE drain (0.3 %/s) close off long approach-and-fire sequences.`)
    recommendations.push(
  ...
  if (r.missionId === 'M05' && stats.sunkIds.length >= 1 && outcome !== 'VICTORY') {
    bugs.push('A ship was sunk but the F9 escape window (detection < 20, nearest escort > 3 km, sustained 30 s) was never satisfied afterwards.')
    recommendations.push(
  ...
  if (bugs.length === 0 && outcome !== 'VICTORY') {
    bugs.push('No isolated anomaly — the session simply did not meet the objective within the tick budget (honest result).')
  }
  if (recommendations.length === 0 && outcome !== 'VICTORY') {
    recommendations.push('Review the mission difficulty target vs. the scripted ceiling evidenced above.')
  }
...
function eventLine(e: { simTime: number; type: string; payload?: Record<string, unknown> }): string {
...
function writeSessionReport(outDir: string, version: string, r: PlaytestResult, def: MissionDef, note?: string): void {
  const findings = collectFindings(r, def)
  const lines: string[] = []
  lines.push(`# Playtest ${pad2(r.session)} — ${r.missionName} (t-014 evidence)`)
  lines.push('')
  lines.push(`- **Version**: ${version}`)
  lines.push(`- **Mission**: ${r.missionId} — ${r.missionName} (seed ${r.seed}, difficulty ${r.difficulty}/5)${note ? ` — ${note}` : ''}`)
  ...
  lines.push('## Actions')
  lines.push('')
  lines.push(`- pings: ${r.actions.pings} · fire inputs: ${r.actions.fireInputs} · moving ticks: ${r.actions.movingTicks} · turning ticks: ${r.actions.turningTicks} · fire rejections (tail): ${r.actions.fireRejections}`)
  lines.push(`- strategy: ${STRATEGIES[r.strategy as StrategyId].description}`)
  lines.push('')
  lines.push('## Result')
  lines.push('')
  lines.push(`- outcome: **${r.outcome}** · score ${r.score.total} (${r.score.grade}) · hull ${r.stats.finalHull} · battery ${r.stats.finalBattery.toFixed(1)}% · detection ${r.stats.finalDetection.toFixed(1)}`)
  lines.push(`- sunk: ${r.stats.sunkIds.length > 0 ? `${r.stats.sunkIds.join(', ')} (${r.stats.sunkClasses.join(', ')})` : 'none'} · damage dealt: ${r.stats.damageDealt.toFixed(1)} hull points`)
  lines.push('')
  lines.push('## Failure')
  lines.push('')
...
function writeSummary(outDir: string, version: string, results: PlaytestResult[]): void {
  const rows: string[] = []
  rows.push('| # | Mission | Seed | Diff | Strategy | Outcome | Duration (s) | Damage | Peak det | Torp hit/fired | Score | Grade |')
  rows.push('|---|---|---|---|---|---|---|---|---|---|---|---|')
  for (const r of results) {
    const eff = r.stats.torpedoesFired > 0 ? `${r.stats.torpedoesHit}/${r.stats.torpedoesFired}` : '—'
    rows.push(
  ...
  lines.push('')
  lines.push(`**Version:** ${version}`)
  lines.push(`**Sessions:** ${results.length} recorded (5 fixed + 5 generated + M01 determinism double-run)`)
  lines.push(`**Victories:** ${victories.length} (${victories.map((v) => v.missionId).join(', ') || 'none'})`)
  lines.push(`**Torpedoes:** ${hit} hits / ${fired} fired (${fired > 0 ? ((hit / fired) * 100).toFixed(0) : '—'}%) · **total damage dealt:** ${damage.toFixed(0)} hull points`)
  lines.push(`**Determinism double-run (M01, seed ${results.find((r) => r.strategy === 'determinism-check')?.seed ?? 1001}):** ${ddIdentical ? 'PASS — final snapshots byte-identical' : 'FAIL or incomplete'}`)
  lines.push('')
  ...
  lines.push('## Notes')
  lines.push('')
  lines.push('- Reports are regenerated by `npm test` (tests/playtest/playtest.test.ts calls runPlaytests()).')
  lines.push('- All numbers are measured from real engine runs (createGame/step, FIXED_DT); no fabricated values.')
  lines.push('- Hard missions (M03+) use best-effort scripts; a scripted loss is recorded as the finding, not masked.')
  lines.push('')
...
function aggregateIssues(results: PlaytestResult[]): { issue: string; evidence: string[] }[] {
  ...
  for (const r of results) {
    ...
    if (r.outcome === 'ERROR') add('Script/engine error in session', `session ${s}: ${r.errors.join('; ')}`)
    if (r.actions.fireRejections > 0) add('Fire inputs rejected (stale/no-target)', `session ${s}: ${r.actions.fireRejections} rejections in event tail`)
    if (r.stats.torpedoesFired > 0 && r.stats.torpedoesHit < r.stats.torpedoesFired) {
      add('Torpedo efficiency < 100 % (misses at effective fire range)', `session ${s} (${r.missionId}): ${r.stats.torpedoesHit}/${r.stats.torpedoesFired} hits at ≤ fire range`)
    }
    if (r.stats.peakDetection >= 40) {
      add('Detection reaches the ≥ 40 band (merchant ALERT / escort escalation threshold)', `session ${s} (${r.missionId}): peak detection ${r.stats.peakDetection}`)
    }
    if (r.stats.finalHull < 100 && r.outcome !== 'VICTORY') {
      add('Escort pressure damages the player (depth charges / deck gun)', `session ${s} (${r.missionId}): final hull ${r.stats.finalHull}`)
    }
    if (r.stats.finalBattery < 15) {
      add('Battery budget pressure (ping 2 % + CRUISE 0.3 %/s)', `session ${s} (${r.missionId}): ${r.stats.finalBattery.toFixed(0)}% remaining`)
    }
    if (r.missionId === 'M05' && r.stats.sunkIds.length >= 1 && r.outcome !== 'VICTORY') {
      add('M05 post-kill F9 escape window unreachable', `session ${s}: 1+ sunk, escape condition never met`)
    }
