 ...
 * headless runner (src/sim/runner.ts). Report files are written only when the
 * caller opts in explicitly (writeReports: true):
 *
 ...
 * reports. `npm test` runs the same battery for its assertions but writes to a
 * temp dir, so tracked evidence is never rewritten as a test side effect.
 *
 * Session plan (12 ≥ 10):
...
export interface PlaytestConfig {
  ...
  outDir?: string;
  /**
   * Write the report files to outDir. Defaults to false: running the battery
   * must not rewrite tracked evidence unless the caller asks for it.
   */
...
function writeSummary(outDir: string, version: string, results: PlaytestResult[]): void {
  ...
  lines.push('');
  lines.push(
    '- Reports are regenerated by `npm run playtest` (explicit opt-in); `npm test` never rewrites them.',
  );
  lines.push(
...
/**
 * Run the full playtest battery and return the session results (audit trail)
 * for the harness assertions. Evidence reports are written only when
 * `writeReports` is true — the default run touches no files.
 */
export function runPlaytests(config: PlaytestConfig = {}): PlaytestResult[] {
  ...
  const writeReports = config.writeReports ?? false;
