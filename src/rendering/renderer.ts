...
export function createRenderer(opts: RendererOptions): Renderer {
  ...
  function render(
    ...
    const playerPos = prev ? lerpPos(prev.playerSub.position, player.position, t) : player.position;
    ...
    for (const enemy of snapshot.enemies) {
      ...
      const pos = prev
        ? lerpPos(prevEnemyPos(prev, enemy.id) ?? enemy.position, enemy.position, t)
        : enemy.position;
    ...
    if (frame.particles) {
      ...
      for (const torp of snapshot.torpedoes) {
        ...
        const pos = prev
          ? lerpPos(prevTorpPos(prev, torp.id) ?? torp.position, torp.position, t)
          : torp.position;
...
function drawMinimap(
  ...
  if (trail !== undefined) {
    for (const [id, points] of trail) {
      ...
      const isEscort =
        enemy !== undefined && (enemy.shipClass === 'Destroyer' || enemy.shipClass === 'Frigate');
  ...
  for (const c of snapshot.contacts) {
    ...
    const est = {
      x: player.position.x + Math.sin(brg) * range,
      y: player.position.y + Math.cos(brg) * range,
    };
  ...
  for (const enemy of snapshot.enemies) {
    ...
    ctx.arc(
      x0 + p.x,
      y0 + p.y,
      Math.max(2, (lkp.errorKm / mapSizeKm) * (size - pad * 2)),
      0,
      Math.PI * 2,
    );
    ctx.stroke();
