...
function createContact(ctx: SystemContext, rt: SonarRuntime, ship: EnemyShip, init: {
  bearingDeg: number
...
export function removeContact(ctx: SystemContext, rt: SonarRuntime, contact: Contact, reason: string): void {
...
function promotionTarget(contact: Contact, track: SonarShipTrack, balance: BalanceConfig): ContactState | null {
...
function evaluatePromotions(ctx: SystemContext, contact: Contact, track: SonarShipTrack, balance: BalanceConfig): void {
  let target = promotionTarget(contact, track, balance)
  let guard = 0
  while (target !== null && target !== contact.state && guard < 8) {
    transitionContact(ctx, contact, target)
    target = promotionTarget(contact, track, balance)
    guard += 1
  }
}
...
function degrade(ctx: SystemContext, contact: Contact): void {
  ...
  const next: ContactState =
    contact.state === 'TRACKED' ? 'CLASSIFIED' : contact.state === 'CLASSIFIED' ? 'SUSPECTED' : 'UNKNOWN'
  contact.state = next
  if (next === 'UNKNOWN') {
...
function refreshContactData(
  ...
  contact.rangeErrorFrac =
    contact.rangeKm === null ? 0 : rangeErrorFracFor(contact.state, pingRangeErrorFrac(track.pingCount, balance))
  ...
  if (contact.state === 'SUSPECTED' || contact.state === 'CLASSIFIED' || contact.state === 'TRACKED' || contact.state === 'CONFIRMED') {
...
export function recordObservation(
  ...
  evaluatePromotions(ctx, contact, track, balance)
