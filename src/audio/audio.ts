...
//   → playable node graph.
// ---------------------------------------------------------------------------
import { AMBIENCE_PARAMS, EVENT_SFX_MAP, SFX_PARAMS } from './sfx';
import type {
  AmbienceParams,
  EventSfxAction,
  FilterSpec,
...
export function buildTorpedoLaunch(ctx: AudioContext, params: SfxParams): AudioGraph {
  ...
  whoosh.connect(bp);
  const whooshEnv = makeEnv(
    ctx,
    t0,
    p(params.gains, 0, 0.5),
    0.01,
    0.1,
    p(params.durations, 0, 0.3),
  );
  bp.connect(whooshEnv);
  const thump = ctx.createOscillator();
  thump.type = 'sine';
  thump.frequency.setValueAtTime(p(params.frequencies, 1, 90), t0);
  const thumpEnv = makeEnv(
    ctx,
    t0,
    p(params.gains, 1, 0.6),
    0.005,
    0.04,
    p(params.durations, 1, 0.08),
  );
...
export function buildTorpedoHit(ctx: AudioContext, params: SfxParams): AudioGraph {
  ...
  const thudEnv = makeEnv(
    ctx,
    t0,
    p(params.gains, 0, 0.9),
    0.005,
    0.15,
    p(params.durations, 0, 0.4),
  );
  ...
  splash.connect(lp);
  const splashEnv = makeEnv(
    ctx,
    t0,
    p(params.gains, 1, 0.7),
    0.01,
    0.15,
    p(params.durations, 1, 0.5),
  );
...
export function buildExplosion(ctx: AudioContext, params: SfxParams): AudioGraph {
  ...
  crackle.connect(bp);
  const crackleEnv = makeEnv(
    ctx,
    t0,
    p(params.gains, 2, 0.4),
    0.005,
    0.1,
    p(params.durations, 2, 0.3),
  );
...
export function buildPeriscopeRaise(ctx: AudioContext, params: SfxParams): AudioGraph {
  ...
  const clunkEnv = makeEnv(
    ctx,
    t0 + clunkAt,
    p(params.gains, 2, 0.35),
    0.004,
    0.05,
    p(params.durations, 1, 0.08),
  );
...
export function buildPeriscopeReady(ctx: AudioContext, params: SfxParams): AudioGraph {
  ...
  const blipEnv = makeEnv(
    ctx,
    t0 + 0.24,
    p(params.gains, 1, 0.18),
    0.005,
    0.04,
    p(params.durations, 1, 0.08),
  );
  ...
  function ensureCtx(): AudioContext | null {
    ...
    if (!Ctor) {
      if (!warnedNoAudioCtx) {
        ...
        console.warn(
          '[audio] AudioContext unavailable — procedural SFX disabled (headless or non-browser runtime).',
        );
