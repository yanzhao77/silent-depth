  ...
  switch (current) {
    case 'NORMAL': {
      ...
      if (t.torpedoNearKm !== null && t.torpedoNearKm <= th.torpedoHeardKm) {
        return {
          next: 'ALERT',
          reason: `torpedo heard at ${t.torpedoNearKm.toFixed(2)}km (${'§7.2 early warning'})`,
        };
      }
    ...
    case 'SUSPICIOUS': {
      if (t.detection >= th.alertDetection) {
        return {
          next: 'ALERT',
          reason: `detection ${t.detection.toFixed(1)} >= ${th.alertDetection}`,
        };
      }
    ...
    case 'SEARCHING': {
      if (!t.huntingDisabled && t.detection >= th.huntingDetection) {
        return {
          next: 'HUNTING',
          reason: `detection ${t.detection.toFixed(1)} >= ${th.huntingDetection}`,
        };
      }
      if (
        !t.huntingDisabled &&
        ...
        return {
          next: 'HUNTING',
          reason: `ping confirm ${t.lastPingHitRangeKm.toFixed(2)}km < ${th.pingConfirmKm}`,
        };
      }
    ...
    case 'HUNTING': {
      ...
      if (t.detection < th.huntingDegradeBelow && timers.huntingBelow40S >= th.huntingDegradeS) {
        return {
          next: 'SEARCHING',
          reason: `detection < ${th.huntingDegradeBelow} for ${th.huntingDegradeS}s`,
        };
      }
    ...
    case 'LOST_CONTACT': {
      ...
      if (t.pingHeard || t.noiseSensed)
        return { next: 'SUSPICIOUS', reason: 're-contact en route' };
